...
- `llp/current/` and `llp/foundation/` hold relative symlinks only. Link a new or in-progress LLP into `current/` while you're working on it and remove the link when done — that is the archive operation. `foundation/` is the small kernel that could recreate the project; promote into it by folding settled decisions into a living document, then linking it. Orient by reading `foundation/`, then `current/`, then the `@ref`s in scope.
...
Skills: orient = context before coding · create = author one LLP · review = LLP 0005 loop, scaled to stakes · adopt = set up LLP in any repo, install/update the skills · maintain = drift / pre-PR / reconcile / retire / curate (promote → archive → realign, LLP 0011.000) · super-refine = dual-model refine-until-ready · ship = tasks done end to end
