...
3. **The lifecycle.** Living documents; `Draft → (Review → Accepted →) Active → Superseded/Tombstoned`; ~~tombstones under `llp/tombstones/`~~ *(amended by [LLP 0011](./0011-llp-v2-overlays-and-sub-llps.rfc.md), 2026-08-22: lifecycle is status-only — path is not lifecycle)*; stale guidance is never left unmarked. Statuses in this repo are corrected to obey this (see Implementation plan, Phase 0).
